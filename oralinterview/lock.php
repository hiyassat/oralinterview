...
require_once(__DIR__ . '/classes/local/audit_service.php');
...
$PAGE->set_url(new moodle_url('/mod/oralinterview/lock.php', ['id' => $cm->id, 'sessionid' => $sessionid, 'mode' => $mode, 'spacui' => $spacui]));
$PAGE->set_title($statuslabel);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($statuslabel);

if ($spacui) {
    oralinterview_spacui_header($cm, 'manage', $statuslabel);
} else {
    echo $OUTPUT->header();
